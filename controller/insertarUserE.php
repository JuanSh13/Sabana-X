...
// POR MEDIO DEL METODO POST
    ...
    if (strlen($identificacion) > 0 && strlen($tipodoc) > 0 && strlen($nombres) > 0 && strlen($apellidos) > 0 && strlen($email) > 0 && strlen($telefono) > 0 && strlen($clave) > 0 && strlen($rol) > 0 && strlen($estado) > 0) {
        ...
        $clavemd=md5($clave);
        // CREAMOS UN OBJETO DE LA CLASE CONSULTAS QUE ESTA EN EL MODELO
        ...
        $result = $objetoConsultas->registraUserE($identificacion, $tipodoc, $nombres, $apellidos, $email, $telefono, $clavemd, $rol, $estado);
